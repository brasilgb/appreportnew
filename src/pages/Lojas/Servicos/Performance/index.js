...
export default function PerformanceMes() {
  const { serPerform, serTotais } = useContext(AuthContext);
  const mesperf = serPerform.map((fatu) => (fatu));
  const mestotais = serTotais.map((fatut) => (fatut));
